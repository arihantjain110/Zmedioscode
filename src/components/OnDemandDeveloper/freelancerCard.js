  ...
  return (
    <div className="border rounded-xl p-4 bg-white flex flex-col gap-2">
      <div className="flex justify-between px-2 py-2">
        <div className="flex justify-between items-center gap-2">
          <Image
            ...
            alt="Picture of freelancer"
            className="rounded-full"
          />
          <div>
            <h2 className="text-sm blur-sm">{user.userName}</h2>
            <p className="bg-[#36B37E29] text-[#1B806A] text-[10px] px-2 py-1 rounded">
              {user.userStatus}
            </p>
          </div>
        </div>
        <div className="flex justify-center items-center">
          <Link href="hire-developer/registration">
            <button className="border border-orange-500 text-orange-500 bg-white text-xs font-semibold px-1 rounded">
              Unlock Developer
            </button>
          </Link>
        </div>
      ...
      <div className="flex justify-between items-center bg-[#F5F5F5] text-xs px-2 py-1 rounded-md">
        <div>
          Experience{" "}
          <span className="font-bold">{`${user.experience} years`}</span>
        </div>
        <div>
          Can join in{" "}
          <span className="font-bold">{`${user.expectedJoining}`}</span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {user.skills.map((skill, i) => (
          <div
            key={i}
            className="flex gap-1 justify-between items-center text-xs bg-[#F3F1FF] px-2 py-1 w-fit rounded-md"
          >
            {skillIconsImage[skill] && (
              <img
                width={14}
                height={14}
                src={skillIconsImage[skill]}
                alt={`${skill} Logo`}
              />
            )}
            <div>{skill}</div>
